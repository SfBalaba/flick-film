...
>>>>>>> 8126b9f30c57fe87c6d4d822bacf574982ce095e
...
def bio(request, id):
    ...
    db = RecommendCos.objects.filter(title=data['alternativeName'])
    var = db[0].recom_movie.split('; ')[:4]
    lst_resp = [
        requests.get(
        f'https://api.kinopoisk.dev/v1.3/movie?selectFields=id&selectFields=name&selectFields=poster.previewUrl&page=1&limit=10&alternativeName={x}',\
    ...
    context = {
        ...
        'recom_movie_list': db[0].recom_movie,

    }
    # return render(request, 'main/movie_base.html',)
